...
// 直覺呈現「目前實際方案」：付費/試用到期 → 已到期（每日 cron 會自動降回免費）；
// suspended / cancelled → 該特殊狀態；其餘 → 方案 / 試用中。
function getPlanPill(
  ...
  // 付費 / 試用已過期：付費功能已即時停用、等每日 cron 降回免費
  if (plan !== "free" && expired)
    return { label: "已到期", cls: "bg-amber-100 text-amber-700" };
  if (status === "trial")
