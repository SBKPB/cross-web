...
function formatDateForApple(date: Date): string {
  return date.toISOString().replace(/[-:]/g, "").replace(/\.\d{3}/, "");
}

export function CalendarButtons({ booking, primaryColor = "#3b82f6" }: CalendarButtonsProps) {
...
END:VCALENDAR`;
  ...
  const handleAppleCalendar = () => {
    const blob = new Blob([appleCalendarData], { type: "text/calendar;charset=utf-8" });
  ...
  return (
    <div className="space-y-3 px-4">
      <p className="text-center text-sm text-slate-500">加入行事曆</p>
      ...
      <div className="flex gap-3">
        ...
        >
          <a href={googleCalendarUrl} target="_blank" rel="noopener noreferrer">
            <Calendar className="mr-2 size-5" style={{ color: primaryColor }} />
            Google 日曆
          </a>
        </Button>
