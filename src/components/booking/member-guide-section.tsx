...
const BIND_BENEFITS = [
  ...
  { icon: RotateCcw, title: "紀錄查詢", description: "隨時查看預約紀錄" },
  { icon: ShieldCheck, title: "專屬提醒", description: "預約異動即時通知" },
] as const;
...
interface MemberGuideSectionProps {
  /** 患者手機號碼，帶到綁定頁 */
  patientPhone?: string;
  /** 診所 ID */
  clinicId?: string;
  /** 預約後的綁定 token，用來自動連結患者資料 */
  bindToken?: string;
}
...
export function MemberGuideSection({ patientPhone, clinicId, bindToken }: MemberGuideSectionProps) {
  ...
  if (bindToken) params.set("bind_token", bindToken);
